        ...
        for doc_id in doc_ids:
            if not doc_id.startswith('.'):  # avoid ".DS_store" file
                ...
                with open(doc_name, 'r', encoding='utf-8') as f:
                    ...
                    # split by most of the special chars (no '\', '.', and '-')
                    # all_words = re.split(r'[`\-=~!@#$%^&*()_+\[\]{};\'\\:"|<,./<>?\s*]', f.read().lower()) # BAD
                    # all_words = re.split(r'[`=~!@#$%^&*()+\[\]{};\\:"|<,/<>?\s*]', f.read().lower())        # GOOD
                    all_words = re.split(r'[`_=~!@#$%^&*()+\[\]{};\\:"|<,/<>?\s*]', f.read().lower())        # test
...
def determine_ret_count(sim_dict: dict, sorted_doc_ids: list):
    ...
    threshold = sim_dict[sorted_doc_ids[0]] - (diff * (30/100))     
...
def count_rel(judgment_dict: dict):
    """
        Count how many documents are judged as relevant.
        i.e. not only judged, but also score > 0
    """
    rel_count = 0
    for score in judgment_dict.values():
        if score > 0:
            rel_count += 1
    return rel_count
...
def begin_evaluation():
    ...
    with open(OUTPUT_FILE_NAME, 'a') as output_file:
        ...
        for query_id, query in standard_queries.items():
            ...
            rel_count = count_rel(judgment_dict)
